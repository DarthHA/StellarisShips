                    ...
                    if (CurrentCooldown <= 0)
                    {
                        ...
                        switch (EquipType)
                        {
                            case ComponentTypes.Weapon_S:
                                scale = 1;
                                DefenseEffectivess = -1.5f;
                                break;
                            case ComponentTypes.Weapon_M:
                                scale = 1.5f;
                                DefenseEffectivess = -3.6f;
                                break;
                            case ComponentTypes.Weapon_L:
                                scale = 2f;
                                DefenseEffectivess = -8.3f;
                                break;
                        }
                        int damage = RandomDamage;
                        bool crit = Main.rand.NextFloat() < Crit / 100f;
                        int protmp = DamageProj.Summon(ship.GetSource_FromAI(), TargetPos.Value, damage, crit, DefenseEffectivess, 0, (int)(36 * scale), (int)(36 * scale), true);
