...
{
    public class TitanBeamUnit : BaseWeaponUnit
    {
        public override string InternalName => "TitanBeam";
            ...
            else                   //进战状态
            {
                ...
                if (target != -1)
                {
                    ...
                    if ((int)CurrentCooldown == 25)
                    {
                        ...
                        int protmp = TitanBeamProj.Summon(ship, RelativePos, Main.npc[target].Center, color, damage, crit);
        ...
        public override void Draw(SpriteBatch spriteBatch, NPC ship, Vector2 screenPos)
        {
            ...
            if (shipNPC.CurrentTarget != -1 && CurrentCooldown <= 135)      //135-115 一级充能，115-95二级充能，95-75三级充能，75-55四级充能，55-35满充能,25激发，0消失
            {
                void DrawMain(Texture2D tex, float alpha)
                {
                    ...
                    spriteBatch.Draw(tex, DrawCenter - screenPos, null, Color.IndianRed * alpha, ship.rotation, new Vector2(tex.Width / 2, tex.Height / 2), 0.75f * shipNPC.shipScale, SpriteEffects.FlipHorizontally, 0);
