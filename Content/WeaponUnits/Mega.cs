...
{
    public class MegaUnit : BaseWeaponUnit
    {
        public override string InternalName => "Mega";
            ...
            else                   //½øÕ½×´Ì¬
            {
                ...
                if (target != -1)
                {
                    if ((int)CurrentCooldown == 15)
                    {
                        ...
                        int protmp = MegaBullet.Summon(ship.GetSource_FromAI(), shipNPC.GetPosOnShip(RelativePos), ShootVel, damage, scaleSlot, crit);
