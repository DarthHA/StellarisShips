...
{
    public class PlasmaBullet : BaseDamageProjectile
    {
        ...
        public override void AI()
        {
            if (Projectile.ai[0] == 0)
            {
                ...
                SomeUtils.AddLightLine(Projectile.Center - Projectile.rotation.ToRotationVector2() * 10 * ModifiedScale, Projectile.Center + Projectile.rotation.ToRotationVector2() * 10 * ModifiedScale, color * 2);
            }
            else
            {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            if (Projectile.ai[0] == 0)
            {
                ...
                {
                    Vector2 scale = new Vector2(1.5f, 1) * 0.15f * k * ModifiedScale;
                ...
                Main.spriteBatch.Draw(Tex1, Projectile.Center - Main.screenPosition, null, Color.White * Projectile.Opacity, Projectile.rotation, OffSet, Projectile.scale * new Vector2(1.5f, 1) * 0.05f * ModifiedScale, SpriteEffects.FlipHorizontally, 0);
        ...
        public override void SafeOnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            if (Projectile.ai[0] == 0)
            {
                SomeUtils.PlaySoundRandom(SoundPath.Hit + "Plasma", 1, Projectile.Center);
                Projectile.ai[0] = 1;
