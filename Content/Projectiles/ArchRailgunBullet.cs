    ...
    public class ArchRailgunBullet : BaseDamageProjectile
    {
        ...
        public static int Summon(IEntitySource entitySource, Vector2 Pos, Vector2 velocity, int dmg, Color color, float scale, bool crit = false, float kb = 0)
        {
            ...
            if (protmp >= 0 && protmp < 1000)
            {
                ...
                (Main.projectile[protmp].ModProjectile as ArchRailgunBullet).ModifiedScale = scale;
                (Main.projectile[protmp].ModProjectile as ArchRailgunBullet).Crit = crit;
            }
            return protmp;
        }
        ...
        public override void SafeOnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            {
                Projectile.ai[0]++;
                int protmp = Summon(Projectile.GetSource_FromThis(), Projectile.Center, Vector2.Zero, Projectile.damage / 4, color, ModifiedScale, Crit);         //一个取巧生成爆炸的方法
                if (protmp >= 0 && protmp <= 1000)
                {
