...
{
    public class ArchMissileProj : BaseDamageProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[0] == 0)
            {
                ...
                else
                {
                    RedirectChance = 0;
                    LastTarget = Target;
                    if (Projectile.timeLeft > 1) Projectile.timeLeft -= 2;
                }

                Projectile.rotation = Projectile.velocity.ToRotation();
                Projectile.Opacity = MathHelper.Clamp(Projectile.Opacity + 0.5f, 0f, 1f);
                ...
                if (Main.rand.NextBool(2))
                {
                    ...
                    Main.dust[num110].scale = 0.5f + Main.rand.NextFloat() * 0.3f;
            ...
            else
            {
                ...
                {
                    for (int i = 0; i < (int)(12 * ExplosionScale * ExplosionScale); i++)
                    {
                        Vector2 vel = (Main.rand.NextFloat() * MathHelper.TwoPi).ToRotationVector2() * (Main.rand.NextFloat() * 4f + 2f) * ExplosionScale;
                        int num110 = Dust.NewDust(Projectile.Center, 0, 0, 303, 0f, 0f, 0, default, 1f);    //73?
                        Main.dust[num110].scale = 0.5f + Main.rand.NextFloat() * 0.4f;
                        Main.dust[num110].position = Projectile.Center;
                        Main.dust[num110].velocity = vel;
                        Main.dust[num110].noGravity = true;
                    }
                }
                SomeUtils.AddLight(Projectile.Center, color, 1.5f * ExplosionScale);
            }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            if (Projectile.ai[0] == 0)
            {
                Texture2D Tex1 = ModContent.Request<Texture2D>("StellarisShips/Images/Common/Missile").Value;
                Texture2D Tex2 = ModContent.Request<Texture2D>("StellarisShips/Images/Common/Missile_1").Value;
                Rectangle rect = new(0, Tex1.Height / Main.projFrames[Projectile.type] * Projectile.frame, Tex1.Width, Tex1.Height / Main.projFrames[Projectile.type]);

                Main.spriteBatch.Draw(Tex1, Projectile.Center - Main.screenPosition, rect, Color.White * Projectile.Opacity, Projectile.rotation + MathHelper.Pi / 2f, rect.Size() / 2f, Projectile.scale * 0.75f, SpriteEffects.None, 0);
                EasyDraw.AnotherDraw(BlendState.Additive);
                Main.spriteBatch.Draw(Tex2, Projectile.Center - Main.screenPosition, rect, color * Projectile.Opacity, Projectile.rotation + MathHelper.Pi / 2f, rect.Size() / 2f, Projectile.scale * 0.75f, SpriteEffects.None, 0);
        ...
        {
            int protmp = Projectile.NewProjectile(entitySource, Pos, velocity, ModContent.ProjectileType<ArchMissileProj>(), dmg, kb, Main.myPlayer);
            if (protmp >= 0 && protmp < 1000)
            {
                Main.projectile[protmp].Center = Pos;
                Main.projectile[protmp].timeLeft = timeLeft;
                (Main.projectile[protmp].ModProjectile as ArchMissileProj).color = color;
                (Main.projectile[protmp].ModProjectile as ArchMissileProj).Crit = crit;
                (Main.projectile[protmp].ModProjectile as ArchMissileProj).DetectRange = detectRange;
                (Main.projectile[protmp].ModProjectile as ArchMissileProj).HomingFactor = homingFactor;
                (Main.projectile[protmp].ModProjectile as ArchMissileProj).MaxSpeed = maxSpeed;
                (Main.projectile[protmp].ModProjectile as ArchMissileProj).Target = target;
                (Main.projectile[protmp].ModProjectile as ArchMissileProj).LastTarget = target;
                (Main.projectile[protmp].ModProjectile as ArchMissileProj).ExplosionScale = explosionScale;
            }
            return protmp;
        }
        ...
        public override void SafeOnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            if (Projectile.ai[0] == 0)
            {
                SomeUtils.PlaySoundRandom(SoundPath.Hit + "Missile", 1, Projectile.Center);
