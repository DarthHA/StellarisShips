    ...
    public class MissileProj : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            if (Projectile.ai[0] == 0)
            {
                ...
                Main.spriteBatch.Draw(Tex1, Projectile.Center - Main.screenPosition, rect, Color.White * Projectile.Opacity, Projectile.rotation + MathHelper.Pi / 2f, rect.Size() / 2f, Projectile.scale * 1.5f, SpriteEffects.None, 0);
                EasyDraw.AnotherDraw(BlendState.Additive);
                Main.spriteBatch.Draw(Tex2, Projectile.Center - Main.screenPosition, rect, color * Projectile.Opacity, Projectile.rotation + MathHelper.Pi / 2f, rect.Size() / 2f, Projectile.scale * 1.5f, SpriteEffects.None, 0);
        ...
        public static void Summon(IEntitySource entitySource, Vector2 Pos, Vector2 velocity, int dmg, Color color, float explosionScale = 1f, float maxSpeed = 10, float detectRange = 1000, int target = -1, int timeLeft = 480, float homingFactor = 0.05f, bool crit = false, float kb = 0)
        {
            ...
            if (protmp >= 0 && protmp <= 1000)
            {
                ...
                (Main.projectile[protmp].ModProjectile as MissileProj).color = color;
                (Main.projectile[protmp].ModProjectile as MissileProj).Crit = crit;
                (Main.projectile[protmp].ModProjectile as MissileProj).DetectRange = detectRange;
                (Main.projectile[protmp].ModProjectile as MissileProj).HomingFactor = homingFactor;
                (Main.projectile[protmp].ModProjectile as MissileProj).MaxSpeed = maxSpeed;
                (Main.projectile[protmp].ModProjectile as MissileProj).Target = target;
